...
const catalog2 = [{ id: 2166, uid: "category_2166", name: "Другое" }];
...
console.log ('\n========================================================================')
console.log('\t\tЗ А Д А Ч А  1    Вывести список (id, name) каталогов')
console.log ('========================================================================\n')

let array1  = catalog.map(getIdName)

function getIdName(item){
 let idAndName = {};
 idAndName.id = item.id;
 idAndName.name = item.name;
...
console.log ('\n\n========================================================================')
console.log('\t\tЗ А Д А Ч А  2      Вывести все каталоги и под ним список подкаталогов с id и названием (подкаталог это childlist) ')
console.log ('========================================================================\n')


let array2  = catalog.map(getIdNameAll)

function getIdNameAll(item){
  let idAndNameAll = {};

  idAndNameAll.id= item.id
  idAndNameAll.name= item.name
  if(item.childList!=0){
 idAndNameAll.childIdName =  item.childList.map(idChild)
  }
...
function idChild(item){
let childIdName = {};
childIdName.id = item.id
childIdName.name = item.name
...
console.log(array2)
  ...
  console.log ('\n\n========================================================================')
  console.log('\t\tЗ А Д А Ч А  3       Вывести все каталоги, подкаталоги, а также список подкаталогов из подкаталогов')
  console.log ('========================================================================\n')


let array3  = catalog.map(getAll)

 function getAll(item){
  let allCatalog = {}
  allCatalog.id = item.id
  allCatalog.uid = item.uid
  allCatalog.name = item.name
  allCatalog.imageUrl = item.imageUrl

  if (item.childList != 0){
    allCatalog.childList = item.childList
  }
...
console.log(array3)
  ...
  console.log ('\n\n==========================================================================')
  console.log('\t\tЗ А Д А Ч А  4             Выведите список с такими параметрами (id:2166,uid:"category_2166", name:"Другое" ) ')           
  console.log ('==========================================================================\n')

let array4 = catalog2.filter(getAllCatSec)
function getAllCatSec(item){
    return item
